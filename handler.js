      ...
      for (let name in global.plugins) {
        ...
        if ((usedPrefix = (match[0] || '')[0])) {
          ...
          if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
            this.reply(m.chat, `Maaf limit kamu habis, silahkan beli melalui *${usedPrefix}buy*`, m)
...
global.dfail = (type, m, conn) => {
  let msg = {
    ...
    unreg: '── 「 NOT REGISTERED 」 ──\nSilakan Register Terlebih Dahulu Sebelum Menggunakan Bot.Cara Register Cukup Dengan Command *.verify* Atau *.daftar namamu.umurmu* \n\nNote: \nHarap Save Serial Number Mu Agar Bisa Melakukan Unreg Database Bot', 
